...
- Last updated: 2025‑09‑20 (Stories 7, 8, 9 complete: legacy MembershipRole column removed; bitmask constraints (nonzero + valid subset) added; final test alignment uses NO_FLAGS for legacy-only invite payload; documentation updated in SnapshotArchitecture & storyLog; rollback script/tag pending Story 10. Prior: removed deprecated MembershipRole enum and obsolete convergence test; flags-only authorization & management complete; avatar pipeline simplification; denomination presets; profile/guardrails; change password; tenant settings/logo; Node 20 testing.)
