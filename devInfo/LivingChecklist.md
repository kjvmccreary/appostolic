...
- Last updated: 2025‑09‑19 (Story 4: Deprecated legacy invite `role` write path — invites now require roles[] or rolesValue; regression tests added; docs updated. Transitional: member role change still accepts legacy role pending next deprecation step. Prior: numeric roles bitmask support; admin gating tightening; removal of legacy web fallback; avatar pipeline simplification; denomination presets; profile/guardrails; change password; tenant settings/logo; Node 20 testing.)
