...
- Metrics: Auth counters (login success/failure, refresh success/failure, rotation, reuse denied, logout single/all, admin forced logout requests/sessions revoked, plaintext emission TEMP), histograms for login/refresh latency, refresh limiter evaluation histogram, key rotation counters (`auth.jwt.key_rotation.tokens_signed{kid}`, `auth.jwt.key_rotation.validation_failure{phase}`), session metrics (`auth.session.enumeration.requests{outcome=success|disabled}`, `auth.session.revoke.requests{outcome}`), security events (`auth.security.events_emitted{type}`), tracing enrichment (`auth.trace.enriched_spans{span_kind,outcome}`). (`auth.refresh.plaintext_suppressed` retired in Story 31.)
