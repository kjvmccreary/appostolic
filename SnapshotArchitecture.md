...
- IAM — Final legacy role cleanup & test alignment (2025-09-20)
  ...
  - Auth/JWT — Story 5b HTTPS Secure Refresh Cookie E2E Harness complete (2025-09-20) — deterministic Secure attribute validation under real TLS.
    ...
    - Auth/JWT — Story 7 Logout & Global Revocation (KICKOFF 2025-09-20)
      - Planned endpoints: `/api/auth/logout` (single refresh token revoke + cookie clear) and `/api/auth/logout/all` (bulk revoke all user refresh tokens + TokenVersion increment for instant access invalidation). Idempotent 204 responses; structured error codes for missing/invalid tokens; leverages existing token versioning instead of tracking access token IDs.
